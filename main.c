...
for (;;)
{
        ...
        dimled = light;
        //use the light reading range limits 50-900, and conver them to 0-100%
        dimled = ((dimled- 50)*100)/(900- 50); if(dimled <= 5)dimled = 0; else if (dimled >=99)dimled = 100;
        fadeLED(dimled);

        //light Controlling LED1
        //Create dead zone of no action to avoid flickering (switching on/off over a a small fluctuating value on light
...
// beginning of area for all students changes
        ...
        //use a dead zone for no action between 1.01-1.03 of the temoroom value
        if(temp < temproom * 1.05 && temp > temproom * 1.01) {
        ...
        //use a dead zone for no action between 0.07-0.09 of the value temp
        ...
        else
        {
            ...
            if(touch >= touchroom * 0.9) {touchyTouchy=0;}  // off
            if(touch <= touchroom * 0.7&&(touchyTouchy==0)) {
                P2OUT ^=  BIT0;
            touchyTouchy=1;
            __delay_cycles(200);
            }  // on

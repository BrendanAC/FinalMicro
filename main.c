...
/**/
void LightOn(int x){
    //Light that you want to turn on
    clear();
    if(x==0){TopLeftToggle;}
    if(x==1){TopMiddleToggle;}
    if(x==2){TopRightToggle;}
    if(x==3){MiddleLeftToggle;}
    if(x==4){CenterToggle;}
    if(x==5){MiddleRightToggle;}
    if(x==6){BottomLeftToggle;}
    if(x==7){ BottomMiddleToggle;}
    if(x==8){ BottomRightToggle;}
